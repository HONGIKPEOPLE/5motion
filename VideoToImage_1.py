...
def process_video_range(video_folder, start_index, end_index):
    ...
    frame_interval = 2  # Set frame_interval to 1 for continuous frames
    starting_frame_number = 0  # Set the starting frame number to 0
    ...
    with open(csv_file_path, 'r') as csv_file:
        ...
        for row in csv_reader:
            ...
            for frame_num in range(start_frame, end_frame + 1, frame_interval):
                frame_num_save = frame_num + starting_frame_number
                video_file_name = os.path.splitext(file_name)[0]
                frame_folder = os.path.join(image_root_dir, video_folder, f'frame{frame_num_save:03d}')
                if not os.path.exists(frame_folder):
                    os.makedirs(frame_folder)

                frame_path = os.path.join(frame_folder, f'{video_file_name}_{frame_num_save:03d}.png')
...
def main():
    ...
    start_index = 1  # 시작 인덱스 지정
    end_index = 1  # 끝 인덱스 지정
